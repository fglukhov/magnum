...
// 	});
	...
	$("[data-target='#portModal']").on("click", function () {
		...
		$("#port_product").val($(this).closest(".port-tmb").find("h3").text());
	...
	$("#consultForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				url: baseUrl + "order.php",
				data: {
					subject: "Банкротство - консультация",
	...
	$("#pollForm").submit(function() {
		if ($(this).valid()) {
			...
			$.ajax({
				...
				data: {
					subject: "Банкротство - результат квиза",
					name: $("#poll_name").val(),
...
$(function () {
	...
	function updateOrderList () {
		...
		for(let idx in cart.data) {
			let item = '<li><button type="button" title="РЈРґР°Р»РёС‚СЊ РёР· РєРѕСЂР·РёРЅС‹" data-action="remove-from-cart" data-id="' + idx + '"></button>' + cart.data[idx] + '</li>';
			list += item;
